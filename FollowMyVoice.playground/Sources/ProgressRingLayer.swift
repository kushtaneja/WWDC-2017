...
/**
 ...
 This class is not interacted with, instead properties are set in ProgressRingView
 and those are delegated to here.
 
 */
class ProgressRingLayer: CAShapeLayer {
    
    // MARK: Properties
    
    /**
     The NSManaged properties for the layer.
     These properties are initialized in ProgressRingView.
     They're also assigned by mutating ProgressRingView.
     */
   ...
   open func pause(){
        pausedTime = self.convertTime(CACurrentMediaTime(), from: nil)
        self.speed = 0.0
        self.timeOffset = pausedTime
    }
    open func resume(){
        let pauseTime: CFTimeInterval = self.timeOffset
        self.speed = 1.0
        self.timeOffset = 0.0
        self.beginTime = 0.0
        let mediaTime: CFTimeInterval = CACurrentMediaTime()
        let timeSincePause: CFTimeInterval = self.convertTime(mediaTime, from: nil) - pauseTime
